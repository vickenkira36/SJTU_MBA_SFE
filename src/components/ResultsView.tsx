...
export default function ResultsView({
  ...
  constraints,
  historicalAssignments: allHistoricalAssignments,
  selectedProvinces,
  onBack,
  onRestart,
}: ResultsViewProps) {
  // 按选中省份过滤数据，未选中省份的数据不参与结果展示
  const hospitals = useMemo(() =>
    selectedProvinces ? allHospitals.filter(h => selectedProvinces.includes(h.province)) : allHospitals,
    [allHospitals, selectedProvinces]
  );
  const territories = useMemo(() =>
    selectedProvinces ? allTerritories.filter(t => selectedProvinces.includes(t.province)) : allTerritories,
    [allTerritories, selectedProvinces]
  );
  const historicalAssignments = useMemo(() => {
    if (!allHistoricalAssignments) return undefined;
    if (!selectedProvinces) return allHistoricalAssignments;
    // 只保留选中省份辖区的历史分配
    const validTrtyCodes = new Set(territories.map(t => t.trtyCode));
    return allHistoricalAssignments.filter(ha => validTrtyCodes.has(ha.trtyCode));
  }, [allHistoricalAssignments, selectedProvinces, territories]);
      ...
      {result.details.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 p-4 mb-6">
          ...
          <div className="space-y-2">
            {result.details.filter(d => !d.startsWith('[SA 诊断')).map((detail, i) => (
            ...
            {result.details.some(d => d.startsWith('[SA 诊断')) && (
              <div className="mt-3 pt-3 border-t border-gray-100">
                <p className="text-xs font-medium text-gray-400 mb-2">SA 收敛诊断</p>
                {result.details.filter(d => d.startsWith('[SA 诊断')).map((detail, i) => (
                  <div key={`sa-${i}`} className="text-xs text-gray-400 font-mono leading-relaxed">
                    {detail}
                  </div>
                ))}
              </div>
