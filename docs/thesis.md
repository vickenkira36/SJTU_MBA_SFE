...
调研对象覆盖 SFE 管理者（区域经理、省经理、大区经理）、一线医药代表、SFE 数据分析师以及销售运营或咨询顾问四类岗位。受访者由作者通过行业人脉招募，每位受访者先完成 16 道封闭式问卷（10-12 分钟），再接受 30-45 分钟的开放式访谈。问卷与访谈在同一轮约访中完成，共回收 12 份有效问卷与 12 位受访者的访谈记录。
...
| 维度 | 分布 |
|------|------|
| 角色 | 销售管理者 3 名 / 一线医药代表 4 名 / SFE 分析师 3 名 / 销售运营或咨询顾问 2 名 |
| 工作年限 | <3 年 1 名 / 3-5 年 4 名 / 5-10 年 5 名 / >10 年 2 名 |
| 企业类型 | 跨国制药企业 6 名 / 国内大型制药企业 4 名 / 国内中小型制药企业 2 名 |
| 产品线 | 处方药专科 5 名 / 处方药普药 4 名 / OTC 1 名 / 多产品线 2 名 |
| 辖区数量经验 | 1-3 个 2 名 / 4-10 个 5 名 / 11-30 个 4 名 / >30 个 1 名 |

![图 3-4 受访者背景分布](figures/fig3-4.png)

数据来源：作者整理调研问卷 A1-A5 题。

样本覆盖管理与一线两类视角、跨国与本土两类企业，从业年限以 5-10 年的中坚岗位为主。受样本规模所限，本调研定位为辅助性的探索性研究，结果以描述性统计形式呈现（均值、入选率、频次分布），不进行推断性统计检验；原始问卷数据已脱敏处理，受访者身份不予披露。完整问卷题本与访谈大纲见附录 A。

## 3.3 调研结果分析

### 3.3.1 综合价值指数 Index 的构成共识

调研第一组题目（B1+B2）聚焦"Index 应由哪些维度构成、各占多少权重"。B1 多选题结果显示，销量与市场潜力是受访者公认的核心维度，入选率分别为 92% 与 83%；HCP 数量与医院市场份额作为补充维度（58% / 50%）；医院等级、客户级别、地理可达性、产品矩阵贡献等其余维度入选率均低于 50%（图 3-5 上层）。

![图 3-5 Index 构成维度入选率与赋权均值](figures/fig3-5.png)

数据来源：作者整理调研问卷 B1-B2 题。

B2 固定和权重题在被勾选条件下进一步收敛了各维度的相对重要性：销量平均赋权 52%、潜力 32%、HCP 数量 10%、医院市场份额 5%，其余维度合计低于 5%（图 3-5 下层）。两道题共同支撑了一个明确结论——业务侧公认 Index 应以销量与潜力为核心二维结构，二者合计占比 84%；HCP 数量与市场份额作为补充信号，可在数据可得性允许时纳入潜力子指标的计算。这一调研结论与第四章 4.2 节的 Index 设计高度一致：本研究构建的 Index 采用销量 60% + 潜力 40% 的简化结构，比例与调研均值 52:32 在四舍五入到 60:40 整数刻度上吻合；HCP 数量与医院市场份额则通过 IQVIA 流向数据与标杆指标系数折算到潜力子指标。
...
**大医院虚拟拆分阈值（D1）。** 50% 受访者认为大医院应在 Index 超出辖区平均值 1.5 倍时即触发拆分，25% 倾向于更保守的 2 倍，17% 倾向于更激进的 1.2 倍（图 3-7）。多数派 1.5 倍取值与本研究第四章 4.3 节的设定一致；少数派的分歧主要源于对"大医院共岗管理"的实操担忧——访谈 I5 反馈中，多位受访者提到绩效分摊与责任边界界定的复杂度，认为应以保守阈值降低拆分频次。

![图 3-7 大医院虚拟拆分阈值频次分布（D1）](figures/fig3-7.png)

数据来源：作者整理调研问卷 D1 题。

**单代表覆盖城市数上限（D4）。** 50% 受访者选择 2-3 个城市，33% 选择 4-5 个，仅 8% 接受 5 个以上的跨大区覆盖（图 3-8）。累计 83% 的受访者将上限收敛在 5 个城市以内，体现了一线代表对差旅半径与拜访频次平衡的实际感受——超出 5 个城市意味着每个城市的有效覆盖时间被严重摊薄。本研究第四章 4.4.2 节的城市上限取值 3-5 个，与多数派意见一致。

![图 3-8 单代表覆盖城市数上限分布（D4）](figures/fig3-8.png)

数据来源：作者整理调研问卷 D4 题。

**单辖区 Index 上下浮动容忍带（D2）。** 50% 受访者认为 ±15% 的辖区 Index 偏差可接受，33% 接受 ±20%，17% 仅接受 ±10%（图 3-9）。累计 83% 的受访者将容忍带集中在 ±10% 至 ±20% 区间，本研究第四章 4.4.3 节采用的 ±20% 阈值（即 200 个 Index 点）位于多数派的容忍上界——这一选择在保留充分均衡优化空间的同时，避免对算法施加过紧的硬性约束。
...
调研第四组题目（E1）让受访者对辖区调整时六类客户关系的"必须保护程度"逐行打分。结果呈现出清晰的两层结构（图 3-10）：

- **必须保护层（均值≥4.5）**：一线代表与 KOL 客户的关系（4.7）、重点大医院（Index 排名前 10%）的代表-医院关系（4.6）、历史只属于一个辖区的城市的代表-城市关系（4.5）。
- **可调整层（均值 3.0-3.4）**：同辖区非 KOL 普通客户的关系（3.4）、普通医院（中等 Index）的代表-医院关系（3.2）、跨多个辖区的城市内代表-医院关系（3.0）。

![图 3-10 客户关系差异化保护程度均值（E1）](figures/fig3-10.png)

数据来源：作者整理调研问卷 E1 题。

两层均值差距 1.3-1.7 个 Likert 分。访谈中的定性观察进一步验证这一分层：多位受访者反馈"大医院和 KOL 关系一旦中断，恢复需要 6 个月以上的客户重建，业绩损失明显；普通医院的代表更换则相对容易，3 个月内即可恢复正常"。这种"高价值关系严守、低价值关系可让步"的业务直觉，对应到算法层面，即第四章 4.5.3 节 Hungarian 权重矩阵的两层差异化设计：医院级权重采用 $\text{idx}_h^2/500$ 的非线性形式让 Index 高的医院在匹配中天然主导，自动实现重点大医院与 KOL 关系的"严守"；一对一城市保护权重提升至 $10^{12}$ 量级，对应"历史独占关系强制冻结"。中等价值医院与小区域关系的权重相对低，使其能够在匹配中被适度让渡，服务于第一阶段 Index 均衡与地理紧凑的双重优化。

### 3.3.5 算法接受度与人机协同的偏好

调研最后一组题目（G1，三组成对比较）测量受访者在算法决策上的方案偏好。结果如图 3-11 所示：

- 第一组（Index 完美均衡 vs 客户保留率 92%）：67% 选择保留率优先；
- 第二组（地理跨度大 vs 紧凑度优先）：58% 选择紧凑优先；
- 第三组（算法黑盒 vs 算法 + 人工调整）：83% 选择人机协同。

![图 3-11 G1 三组成对比较结果](figures/fig3-11.png)
...
把 3.3 节的五组分析结果汇总，可以从业务侧凝练出一份明确的算法需求清单（表 3-2），每条需求都对应到第四章模型构建的一个具体设计：
...
| 大医院应在 Index 超出 1.5 倍时被多人共同覆盖 | D1（50% 选 1.5x） | 4.3.1 节虚拟拆分阈值 1.5 倍 |
| 单辖区 Index 偏差容忍带 ±10%~±20% | D2（83% 集中在该区间） | 4.4.3 节 Index 偏差软约束阈值 ±20%（200 点） |
| 单代表覆盖城市数 ≤5 | D4（83% 收敛 ≤5 城市） | 4.4.2 节城市上限硬约束 3-5 |
| 客户关系应分层保护（高价值严守、低价值可调整） | E1（必须保护层均值≥4.5 vs 可调整层 3.0-3.4） | 4.5.3 节 Hungarian 权重 $\text{idx}^2/500$ + 一对一保护 $10^{12}$ |
...
本章先以简练笔触回顾了制药企业 SFE 辖区管理面对的政策三重压力（集采常态化、医保谈判机制化、医药反腐深水化）以及现行手工与商业化 SaaS 两条路径的局限性，确认这些痛点不是个别企业的孤立问题；继而通过一轮针对 12 位行业从业者的深度访谈与问卷调研，从业务侧验证了痛点的集体性，并把分散的诉求收敛为一份明确的算法需求清单。需求清单的每一条都对应着第四章模型的一个具体设计——从 Index 二维构成、关键阈值取值，到客户关系差异化保护、两阶段解耦、人机协同接口。下一章将基于这份需求清单，从数学建模和算法设计的角度系统化构建两阶段智能辖区分配模型。
...
1.5 倍这一阈值的取值，在调研中获得了多数业务受访者的认可。问卷 D1 题显示，50% 的受访者认为大医院应当在 Index 超出辖区平均值 1.5 倍时即触发虚拟拆分，25% 倾向于更保守的 2 倍阈值，17% 倾向于更激进的 1.2 倍阈值——多数派与本研究取值一致；少数派的分歧主要源于对绩效分摊与共岗管理复杂度的担忧（详见 3.3.3 节及附录 A 访谈 I5 的反馈）。算法层面采用 1.5 倍属于折中点：既能避免过低阈值导致大量医院被拆分、增加日常管理负担，又能确保所有显著超出理想值的"巨头医院"都被妥善分散。详细分布见 3.3.3 节图 3-7。
...
**城市上限约束。** 单个辖区覆盖的城市数量不得超过用户设定的阈值（默认值通常为 3-5 个城市）。这一约束的业务含义是控制代表的差旅范围——覆盖过多城市意味着代表需要频繁跨城出差，不仅增加差旅成本，也降低了对每家医院的拜访频次。在 Move 操作中，如果目标辖区已经达到城市上限且待移入的医院来自一个新城市，该操作会被跳过。3-5 个城市的上限取值在调研中得到业务侧的清晰背书：问卷 D4 题显示，50% 的受访者认为单代表合理覆盖的城市数应在 2-3 个，33% 选择 4-5 个，仅 8% 接受超过 5 个的跨大区覆盖——累计 83% 的受访者将上限收敛在 5 个城市以内。这一结果反映了一线代表对差旅半径与拜访频次平衡的实际感受：超出 5 个城市意味着每个城市的有效覆盖时间被严重摊薄。详细分布见 3.3.3 节图 3-8。
...
两阶段解耦的方法论选择，在调研中得到了业务侧明确的偏好支持。问卷 G1 题以三组成对方案让受访者表态：第一组中，方案 A "Index 完美均衡但客户保留率仅 75%" 与方案 B "Index 偏差 ±15% 但客户保留率 92%" 之间，67% 受访者选择 B；第二组中，方案 A "地理跨度大但 Index 完美均衡" 与方案 B "地理紧凑但 Index 偏差 ±10%" 之间，58% 受访者选择 B；第三组中，方案 A "算法黑盒输出全局最优" 与方案 B "算法 + 人工调整" 之间，83% 受访者选择 B。前两组结果说明业务侧不接受将均衡性作为压倒性目标而牺牲客户关系或地理紧凑度——单阶段方法把所有目标揉进一个代价函数后，均衡性容易在权重失衡时压倒其他目标，正是受访者所抵触的情形；两阶段解耦的"先纯净均衡、后独立匹配"恰好规避了这一陷阱。第三组结果则为第六章将要展开的"人机协同"商业化设计做了铺垫。详细分布见 3.3.5 节图 3-11。
...
医院级权重采用 Index 平方项的设计，对应着调研中清晰的客户关系差异化保护层级。问卷 E1 题让受访者对六类客户关系的"必须保护程度"逐项打分（1-5 分制），结果呈现明显分层：一线代表与 KOL 客户的关系均值 4.7、重点大医院（Index 排名前 10%）的代表-医院关系均值 4.6、历史只属于一个辖区的城市的代表-城市关系均值 4.5，三类高价值关系均值≥4.5，构成"必须保护层"；同辖区非 KOL 客户关系均值 3.4、普通医院关系 3.2、跨多辖区城市内的代表-医院关系 3.0，构成"可调整层"。两层均值差距约 1.3-1.7 个 Likert 分。这一"高价值关系严守、低价值关系可让步"的业务直觉，对应着权重矩阵的两层差异化设计：医院级权重的 $\text{idx}_h^2/500$ 让 Index 高的医院在匹配中天然占据主导地位，从而自动实现重点大医院与 KOL 客户关系的"严守"——一家 Index 为 800 的医院贡献的权重相当于 16 家 Index 为 200 的医院之和，匹配的最优解会优先围绕大医院展开；下文 4.5.3.4 节将进一步介绍的一对一城市保护权重 boost 至 $10^{12}$ 量级，则对应"历史独占关系强制冻结"。中等价值医院与小区域关系在两个权重中的占比相对较低，使其能够在匹配中被适度让渡，服务于第一阶段已经达成的 Index 均衡与地理紧凑性目标。详细打分分布见 3.3.4 节图 3-10。
...
附录 A 收录第三章 3.2 节所述深度访谈与问卷调研的完整工具集——A.1 节简要回顾调研方法的关键设定，便于读者快速定位；A.2 节给出 16 道封闭式问卷的完整题本；A.3 节给出 8 道开放式访谈题目。受访者背景与调研结果分析已在第三章 3.2.2 与 3.3 节集中呈现，附录不再重复。

## A.1 调研方法摘要 {.unnumbered}

调研对象为具备 SFE 系统认知的行业从业者，覆盖销售管理者、一线医药代表、SFE 分析师、销售运营/咨询顾问四类岗位。受访者由作者通过行业人脉招募，每位受访者先完成 16 道封闭式问卷（10-12 分钟），再接受 30-45 分钟的开放式访谈。问卷与访谈在同一轮约访中完成，共回收 12 份有效问卷与访谈记录（N=12）。
...
(1) 1-3 个   (2) 4-10 个   (3) 11-30 个   (4) 30 个以上
...
### A.2.5 客户关系维持的差异化处理 {.unnumbered}

E1. 在辖区调整时，应在多大程度上保护以下六类客户关系？请逐行打分，1 = 可以重新分配，5 = 必须保护原有关系。

| 关系类型 | 1 | 2 | 3 | 4 | 5 |
|---------|:-:|:-:|:-:|:-:|:-:|
| 重点大医院（Index 排名前 10%）的代表-医院关系 | ○ | ○ | ○ | ○ | ○ |
| 历史只属于一个辖区的城市的代表-城市关系 | ○ | ○ | ○ | ○ | ○ |
| 一线代表与其辖区内 KOL 客户的关系 | ○ | ○ | ○ | ○ | ○ |
| 普通医院（中等 Index）的代表-医院关系 | ○ | ○ | ○ | ○ | ○ |
| 跨多个辖区的城市内代表-医院关系 | ○ | ○ | ○ | ○ | ○ |
| 同辖区内非 KOL 普通客户的关系 | ○ | ○ | ○ | ○ | ○ |

### A.2.6 场景化优先级 {.unnumbered}
