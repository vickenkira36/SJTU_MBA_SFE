...
**早期采用者画像。** 综合三种形态的客户画像，本研究算法在商业化早期最具突破潜力的目标客户是**销售团队规模 1,000-3,000 人快速扩张期的本土创新药企**——这一阶段企业刚跨过 SFE 必要性阈值但尚未完成自建，是"职能替代"形态最自然的切入窗口。其次是经历集采产品线缩编、需要快速完成销售团队结构性瘦身的中型 MNC——"职能减量"价值在这一情境下最容易被高管层接受。早期推广的核心阻力集中在 6.1 节讨论过的可解释性、数据敏感性、组织变革管理三个层面，对策已在 6.1.1、6.1.2 两小节展开。
...
**不足一：实证样本仅覆盖业内某跨国药企的三省份 BC/LC 数据，单时点快照未能验证算法在跨产品线、跨企业类型场景下的泛化性。** 第五章实证基于业内某跨国药企的 BC（基础医院）与 LC（重点医院）两类产品线、上海/湖南/新疆三省份数据，所有数据脱敏后保存在 data/case/（已 gitignored）。受数据可获得性所限，本研究未能横向比较国内创新药企、Biotech、零售药房等不同业务模式下算法的适配度，也未能纵向追踪同一辖区在多个时点的算法表现。**未来工作**应在合规前提下扩展样本：横向引入 2-3 家不同产品组合与销售队伍规模的企业做对比验证；纵向选定某一企业 1-2 个产品线做季度连续追踪，观察算法在动态市场环境中的方案稳定性与累计客户保留率。

**不足二：算法目标函数的多维权衡仍依赖人为权重设定，未引入业务实际反馈作闭环优化。** 当前 Hungarian 权重（idx²/500 + 区县 +100 + 城市 +50）与 SA 罚函数权重（θ_cv、θ_radius、θ_count 等）来自第三章调研偏好与作者业务经验的混合判断，本研究未做权重的敏感性扫描，也未把"业务侧实际接受/拒绝率"作为反向修正信号。**未来工作**有两个延展方向：一是扩展第五章 5.4 节敏感性扫描到权重维度，给出权重调整对方案的边际影响曲线；二是引入算法 → 业务接受度 → 权重再修正的强化学习闭环，让算法权重从"调研驱动"升级为"业务反馈驱动"，形成自适应优化系统。

**不足三：第六章 6.2 节关于"SFE 职能蒸馏 + 垂类 Agent + 外包咨询"行业路径的论述基于趋势研判与作者业务经验，缺少 POC 级商业验证。** 本研究在商业化部分给出三种差异化形态（垂类 Agent 外包、SaaS 订阅、企业内部部署）的适用场景判断，但未与具体药企开展小规模试点验证客户付费意愿、合同条款、续约率等真实商业指标。**未来工作**应分两步推进商业验证：第一步选取本土创新药企（销售队伍 1,000-3,000 人扩张期）做 SaaS 订阅形态的 POC 试点，3-6 个月收集"算法上线 → 销售业绩 → 客户保留率 → 团队接受度"四维数据；第二步对比 SaaS 订阅与垂类 Agent 外包两种形态在不同企业类型下的客户终身价值（LTV）、获客成本（CAC）、流失率，从而验证或修正第六章的形态匹配假设，让"行业转型路径"判断从趋势观察上升为可量化的商业模式。
