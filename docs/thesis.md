...
更重要的是，**86.4% 的 Index 加权保留率突破了第三章 3.3.3 节调研中 D2 题受访者期望的 ≥85% 业务可接受线**。这一结果说明算法在大幅改善 Index 均衡度（CV 56.5% 的改善幅度）的同时，并没有以损失客户关系延续为代价；相反，通过两阶段解耦设计——SA 阶段不被历史锚定、Hungarian 阶段独立最大化保留率——算法实现了"均衡与延续同向优化"的目标。
...
本章基于业内某跨国药企 BC 与 LC 两个产品组的脱敏数据，从三个层次系统验证了第四章构建的两阶段优化算法的实证效果。在主样本验证层面，湖南省 BC 数据的对比显示算法把 Index 变异系数从 44.3% 降至 19.3%（改善 56.5%）、最大辖区 Index 从 2521 收敛到 1348（降幅 46.5%）、辖区平均最大半径从 50.9 km 降至 37.4 km（改善 26.5%）；按 Index 加权的客户保留率达到 **86.4%**，超过第三章 D2 调研期望的 ≥85% 业务可接受线。这一保留率充分体现了"抓大放小"的差异化保护——含全 Index 接近 3250 的省级肿瘤专科医院在内的高价值大客户关系，被 Hungarian 第二阶段匹配以及 4.2.3 节虚拟拆分机制共同保留下来。
...
需要诚实承认两个 trade-off。其一，算法在极度失衡场景（如上海 CV=64%）下做出的实质性结构重构会导致客户关系的部分变动，按医院数计算的保留率（21%-69%）在不同场景下呈现明显分化；只有在中等失衡场景（湖南）下才能达到主流业务可接受的保留率水平（86.4%）。这反映了"均衡与延续的客观权衡"——业务侧偏好的 ≥85% 保留率主要适用于 As-Is 接近主流均衡水平的省份，在极端失衡省份引入算法时需要更长的过渡周期。其二，算法在稀疏场景（新疆）下虽实现了 46.7% 的地理半径改善，但 Index CV 仍维持在 35.3%、超阈辖区占 70%，反映了大跨度低密度场景下均衡优化的客观天花板——这是地理与业务硬约束下的合理边界，而非算法局限。
