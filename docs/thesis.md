...
![图 5-4 三省份计算效率与可扩展性](figures/fig5-4.png)
...
需要诚实承认两个 trade-off。其一，算法在极度失衡场景（如上海 CV=64%）下做出的实质性结构重构会导致客户关系的部分变动，按医院数计算的保留率（21%-69%）在不同场景下呈现明显分化；只有在中等失衡场景（湖南）下才能达到主流业务可接受的保留率水平（86.4%）。这反映了"均衡与延续的客观权衡"——G1 调研中受访者偏好的高保留率方案主要适用于 As-Is 接近主流均衡水平的省份，在极端失衡省份引入算法时需要更长的过渡周期。其二，算法在稀疏场景（新疆）下虽实现了 46.7% 的地理半径改善，但 Index CV 仍维持在 35.3%、超阈辖区占 70%，反映了大跨度低密度场景下均衡优化的客观天花板——这是地理与业务硬约束下的合理边界，而非算法局限。
...
第二类是中层管理者的阻力，核心心理是权力转移焦虑（"以前我说了算，现在算法说了算"）以及历史决策被算法挑战时的面子压力。这类阻力如果用"全自动化"的方式硬推会激化矛盾——管理者会从内心抵触算法。本研究主张通过 6.1.1 节的人机协同机制反向化解：保留管理者对算法约束参数的配置权（拆分阈值、销量潜力权重、城市数上限等业务策略型参数仍由企业按偏好设定）以及对特定医院的强制干预权（锁定 / 拆分 / 排他约束），让管理者从"被算法挑战"的姿态转变为"算法的使用者与受益者"。这一框架与 4.4.3.4 节调研结论一致——业务侧 67%/58% 偏好"客户保留率/紧凑度优于绝对均衡"、83% 偏好"算法+人工调整"，本质上都在表达"我们要保留对关键节点的最终决定权"。
