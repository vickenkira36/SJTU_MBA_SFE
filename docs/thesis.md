...
考虑到具备 SFE 系统认知的从业者总量有限，且调研内容兼具量化判断与定性挖掘的双重性质，本研究采用以深度访谈为主、问卷量化验证为辅的研究方法。深度访谈用以挖掘判断背后的业务逻辑与少数派观点（例如"为什么大医院要被多人共同覆盖"在管理层面的实际挑战）；问卷量化则用以收敛业务侧对关键阈值、权重、优先级的具体判断，作为算法参数取值的实证锚点。这一方法组合在管理学小样本研究中较为常见——例如《S 公司技能型人才的激励优化研究》采用了 16 名访谈对象，《N 公司项目人才管理研究》采用了 8 名访谈对象，本研究的 12 人样本与之相当。
...
| 收缩与扩张场景的优先级应可差异化配置 | F1+F2（多数派分别偏好客户保留率与高潜力覆盖） | 第四章 4.1.1 节三类触发场景差异化处理 + 可配置参数接口 |
...
第四章完成了模型与算法的形式化构建，本章将在此基础上以业内某跨国药企脱敏后的真实业务数据为对象，验证算法的实际效果、地理适应性与运行性能。全章遵循"实验设计→主样本深入→多场景广度→性能与稳健性"的逻辑：首先说明数据来源、实验设置与评价指标体系；随后选取湖南省样本作为主样本，深入对比 As-Is 人工分配与 To-Be 算法分配在 Index 均衡度、地理紧凑性、客户保留率三个维度的差异；继而把样本扩展到上海（聚集型直辖市）与新疆（地理跨度极大的稀疏省份）两个地理形态迥异的场景，验证算法的鲁棒性；最后评估计算效率与可扩展性，并讨论关键参数的敏感性。
...
需要特别说明客户保留率的两种口径：按医院数计算只统计有多少家医院的辖区归属未变，把所有医院视为等权重；按 Index 加权计算则将每家医院的 Index 作为权重，反映"高价值客户关系是否被保留"。两种口径之间往往存在差异——按医院数算保留率较低、按 Index 加权后保留率较高，恰恰说明算法在客户关系保留上呈现"抓大放小"的特征：高 Index 大医院的关系更容易被 Hungarian 匹配优先保留下来，而低 Index 小医院则在均衡优化中被相对自由地重新分配。这种差异化保护与 4.4.3.2 节 Hungarian 医院级权重 $\text{idx}^2/500$ 的非线性设计直接对应。

## 5.2 主样本验证：湖南 的 As-Is 与 To-Be 对比

湖南省样本数据共包含 60 家医院、16 个辖区，是中部经济发达省份的典型代表，也呈现出"省级专科医院 Index 占比畸高、外加多个地市级辖区中等规模"的复合型分布形态。这种分布既包含了高 Index 大医院的处理（验证 4.2.3 节虚拟拆分机制），也覆盖了多地市的常规辖区均衡，对算法的考验更为综合。本节以湖南为主样本，深入展开 Index 均衡度、地理紧凑性、客户关系延续性三个维度的对比分析。
...
数据来源：作者基于业内某跨国药企肿瘤团队湖南省脱敏数据 + 论文项目本地算法（commit cbc1a756）实证生成。
...
数据来源：作者基于业内某跨国药企肿瘤团队湖南省脱敏数据 + 论文项目本地算法实证生成。
...
需要诚实承认的是，新疆 To-Be 的 CV 仍为 35.3%，明显高于上海（13.0%）和湖南（19.3%）；超 ±20% 辖区数仍占 70%。这反映了稀疏场景下均衡度优化的客观天花板——在 35 家医院、Index 总量分布极不均匀（含若干个区域中心三甲医院）、地理跨度数百公里的硬性约束下，算法已经做到了局部最优解。这种"承认边界条件"的诚实呈现，恰恰说明算法不会强行追求"全场景同等优化"，而是依据场景特征做出合理判断——这种自适应能力正是 4.4.2.3 节模拟退火"自适应初始温度"设计的实证体现。
...
第二，算法在不同地理形态下都能在地理紧凑性维度做出大幅改善——这是个超越直觉的发现：聚集场景（上海）半径压缩 59.1%、中等场景（湖南）压缩 26.5%、稀疏场景（新疆）压缩 46.7%。在引入算法之前，业务直觉认为"地理跨度大的省份算法难有作为"——但新疆 的实证证伪了这一假设：即使在 166 万平方公里的大省里，算法仍能识别出地理上自然成簇的医院、把人工分配中被打散的相邻医院重新归并，从而把辖区平均半径从 206 km 压到 110 km。这一能力来自 4.3.1 节阈值惩罚框架的二次惩罚设计——超出合理阈值后惩罚成倍增长，让算法在所有场景下都强烈倾向于"避免地理破碎"，与场景密度无关。
...
从图中可以看到两个性能特征。一是绝对耗时极低——三省份均在 4 秒以内完成完整的两阶段优化（含六层聚类、500K 次模拟退火迭代、Hungarian 匹配），相比人工方案的 4-8 周（约 240-480 小时），算法的耗时缩短了约 5 个数量级。这一性能水平意味着 SFE 团队可以在秒级响应中完成"参数调整-重新优化-结果对比"的迭代循环，从根本上改变了辖区管理的工作模式——从过去"一年一次的重大事件"变为"随时可以进行的日常分析"。

二是耗时与问题规模呈合理增长关系：上海规模（99×19 = 1881）耗时 3.8 秒最长，湖南（60×16）和新疆（35×10）规模较小、均在 1.1 秒——这与 Hungarian 算法 $O(K^3)$ 的复杂度和 SA 阶段 O(N) 的迭代代价相吻合。从工程实用角度看，对于制药企业典型的省级问题规模（K 通常在 5-50 之间，N 在 100-1000 之间），算法的耗时基本控制在 30 秒以内。即便扩展到全国规模（按 30 个省份并行计算），整体耗时也可以控制在 5 分钟以内——这意味着算法不仅适合单省优化场景，也具备支撑全国级辖区一次性重新规划的能力。
...
数据来源：作者基于业内某跨国药企肿瘤团队三省份脱敏数据 + 论文项目本地算法 12 次扫描实证生成。图 5-5 以折线图形式可视化了表 5-3 的扫描结果，直观呈现三省份在不同迭代次数下 To-Be CV 与改善幅度的变化趋势。
...
本章基于业内某跨国药企肿瘤团队的脱敏数据，从三个层次系统验证了第四章构建的两阶段优化算法的实证效果。在主样本验证层面，湖南省样本数据的对比显示算法把 Index 变异系数从 44.3% 降至 19.3%（改善 56.5%）、最大辖区 Index 从 2521 收敛到 1348（降幅 46.5%）、辖区平均最大半径从 50.9 km 降至 37.4 km（改善 26.5%）；按 Index 加权的客户保留率达到 86.4%，与第三章 G1 调研中 67% 受访者偏好"客户保留率优先于绝对均衡"的业务判断方向一致。这一保留率充分体现了"抓大放小"的差异化保护——含全 Index 接近 3250 的省级肿瘤专科医院在内的高价值大客户关系，被 Hungarian 第二阶段匹配以及 4.2.3 节虚拟拆分机制共同保留下来。
...
数据治理与系统集成是算法落地的基础保障。 算法是数据驱动的——数据治理质量决定算法输出可靠性，系统集成深度决定算法能否日常化运行。第四章 4.2.1 节已明确算法所需的多源数据分为 HCO 主数据、业务数据、历史分配数据三类，分别对应不同的更新频率与治理重点（主数据关注 inscode 唯一性与坐标准确性，业务数据关注与 CRM/SFA 月度刷新通道，历史分配数据则需要算法运行的输入参数、算法版本、输出方案三元组留痕——这一审计机制对应 5.1.2 节“实证基于 commit cbc1a756 版本”的版本声明思路，让每次算法决策可追溯可复现）。系统集成可按企业算法成熟度渐进推进——从最初由 SFE 团队人工搬运 Excel 的轻量形态，到算法直读 CRM 主数据、回写 SFA 分配的中度集成，最终演进为算法嵌入 SFA dashboard、业务侧一键触发的深度集成；不同层次对应不同的部署成熟度，企业可按基础设施条件渐进推进。算法操作权限则需按“中央 SFE 团队完整配置权 + 大区经理参数微调与约束施加权 + 地区经理只读”的三级模型分级管理，避免业务策略型参数被随意修改而失去对比基础。
...
| Index 指标体系构建 | 中高（数据归一化与权重模型）| 中高 | 部分覆盖（4.2 节）|
...
形态二：SaaS 平台订阅，对应"职能减量"价值。 适用于已建立成熟 SFE 部门的中型 MNC 和本土头部企业——这类企业不愿完全外包 SFE 职能（出于数据敏感、组织稳定性等考虑），但希望通过算法工具显著降低 SFE 部门的人力负担，把原本数十人 SFE 部门完成的工作量压缩到三分之一规模即可承担。SaaS 形态的关键挑战是 3.1.2 节所述的"水土不服"问题——不同企业、不同产品线、不同省份的业务规则差异需要在标准化与定制化间平衡，本研究算法在 4.4.3 节设计的可配置约束机制（拆分阈值、销量潜力权重、城市数上限等业务策略型参数全部参数化）正是为解决这一矛盾而设计。
...
结论二：六层地理聚类 + SA + Hungarian 的两阶段架构在多目标多约束条件下能稳定输出业务可用方案，且具备跨省份地理形态鲁棒性。 第四章 4.4 节给出了完整的两阶段算法：第一阶段六层地理聚类生成均衡的初始解、SA 在 50 万次迭代内收敛到目标函数最优、Hungarian 算法解决 To-Be 簇与 As-Is 辖区编号的最大权匹配；第五章 5.2 节湖南 主样本实证显示，算法在 Index 均衡性（CV 44.3% → 19.3%）、地理紧凑性（平均最大半径 50.9 → 37.4 km）、客户保留率（86.4% Index 加权）三个维度同步改善，且 5.3 节的三省份对比验证了算法在聚集型（上海）、中等密度（湖南）、稀疏型（新疆）三种地理形态下都能产出可解释、业务可接受的结果——这一鲁棒性是工业落地的前提。
...
创新点一：将"客户保留率"从隐性约束显性化为算法目标维度，并通过 Hungarian 二阶段匹配机制工程化落地。 国际经典 SFE 文献长期以来把"销售员-客户关系连续性"视作算法之外的辅助考虑，要么完全忽略、要么以软性建议形式留给业务侧手工干预。本研究在第四章 4.3 节首次将客户保留率拆解为可计算的二阶段优化目标——第一阶段产出"匿名簇"专注均衡性与紧凑性、第二阶段用 Hungarian 算法在簇与历史辖区编号之间求最大权匹配最大化保留率（权重函数为 idx²/500 + 区县 +100 + 城市 +50 + 一对一城市 10¹²）。这一解耦机制让"质量与连续性"两个原本相互掣肘的目标在算法中并行兼顾，是对经典文献的实质性扩展。

创新点二：完整覆盖"政策/行业 → 调研 → 算法 → 实证 → 商业化"的全链条 MBA 视角分析，弥补国内文献在工程化与管理化两端的双重缺失。 国内既有运筹算法研究（杨磊 2012、罗军等 2022、马傲冬等 2026）多聚焦算法收敛性与求解性能，业务侧落地讨论极少；国内 SFE 管理研究（孙旭然 2022、徐蓉 2023、陈兆兆 2024）则停留在制度与流程描述，缺少算法工具支撑。本研究试图在两者之间搭建桥梁——以第三章 12 位业务专家深度访谈与 14 题问卷调研锚定真实业务约束、以第四章 4.2 节多源数据清洗与第五章 5.2-5.4 节三省份实证给出可复现的算法实现，再以第六章 6.2 节"SFE 职能行业转型"的商业化论述把研究从"工具开发"提升到"行业范式判断"。这一全链条结构正是 MBA 学位论文区别于纯工科或纯管理论文的核心增量价值。
